...
`vef init`, `vef migrate`, `vef project`, `vef validate`, and the read-only `vef doctor` manage adoption, canonical storage, projections, and integrity. The explicitly mutating `vef doctor --fix` orchestrates supported remediation. The read-only query interface is `vef list`, `vef show`, `vef refs`, `vef why`, `vef graph`, and `vef search`. Text output is the human interface; `--json` uses a versioned envelope for automation. `why` follows declared task→roadmap→vision and decision-rationale edges without model interpretation.

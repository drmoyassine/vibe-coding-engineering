...
Legacy monolithic repositories remain readable during the compatibility window. The read-only `vef doctor` classifies the core as not adopted, semantically blocked, structurally repairable, or enforced. `vef doctor --fix` is explicit authorization for supported mechanical repair: it preflights the complete record graph, migrates or relocates storage, projects ledgers, validates strictly, and reruns doctor. Semantic ambiguity is reported precisely and blocks every write. The storage migration refuses conflicting partial directories, uses recoverable per-file replacement, writes the manifest last, and can be rerun after an interrupted pre-manifest migration.

Core enforcement and agent-adapter compatibility are separate dimensions (DEC-007). Existing adapters are consumer-owned and never overwritten; `doctor --fix` may install only absent adapter files as a convenience. Adapter attention is visible but cannot make an otherwise valid core fail. The retired `--update-adapters` option rejects the request without changing files. Lower-level `migrate`, `project`, and `validate` operations remain available for advanced use and CI, but adopting users need only `doctor` and `doctor --fix`. Package resolution stays outside the project mutation boundary—an obsolete installed CLI cannot execute future remediation code. Until TASK-001 publishes VEF to npm, commit-pinned consumers must update their dependency before invoking `npx vef doctor --fix`; registry-based `@latest` instructions are not valid yet.
...
The consumer-facing lifecycle is deliberately small: `vef init`, read-only `vef doctor`, and explicitly mutating `vef doctor --fix`. Advanced `vef migrate` and `vef project` commands expose the underlying storage mechanics; `vef validate` is the CI/integrity gate. The read-only query interface is `vef list`, `vef show`, `vef refs`, `vef why`, `vef graph`, and `vef search`. Text output is the human interface; `--json` uses a versioned envelope for automation. `why` follows declared task→roadmap→vision and decision-rationale edges without model interpretation.

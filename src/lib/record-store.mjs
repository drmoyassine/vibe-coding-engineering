...
function generatedItems(layout, items) {
  ...
  const banner = `<!-- Generated by VEF from ${layout.directory}/. Edit canonical item files and ${layout.directory}/${COLLECTION_TEMPLATE}, then run \`vef setup\`. -->`;
  ...
  for (const layout of layouts) {
    ...
    if (template !== null) {
      try {
        ...
        if (stale) projectionIssues.push(`${layout.ledger} is missing or stale; run vef setup`);
...
export async function projectLedgers(projectDir = '.', { write = false } = {}) {
  ...
  if (loaded.storage.mode !== 'per-item') throw new Error('Per-item storage is not active; run vef setup first');

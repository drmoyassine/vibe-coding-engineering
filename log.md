...
- Passed all 60 tests, strict dogfood enforcement, diff checks, and an isolated install of the exact packed artifact. TASK-035 now owns versioning, cross-platform release-gate evidence, trusted staging, human 2FA approval, registry hash/provenance verification, and public-registry acceptance before bugs #3 through #5 close.
- Prepared version 0.3.1 and expanded the packed-artifact acceptance gate to exercise every correction from an isolated consumer. The complete release check passed with 37 expected package files, dry-run SHA-1 `32b1aa5ec6beab0e304b609eba350771aaf3f50d`, and a matching `v0.3.1` version/tag contract; publication remains pending trusted staging and human approval.

### Independent QA findings converted into ordered durable work
- Created canonical GitHub bugs [#3](https://github.com/drmoyassine/vibe-engineering-framework/issues/3), [#4](https://github.com/drmoyassine/vibe-engineering-framework/issues/4), and [#5](https://github.com/drmoyassine/vibe-engineering-framework/issues/5) for the confirmed malformed-lease write blocker, missing roadmap ID allocation, and unnecessary proposal-file requirement during title-authority repair.
- Added FRAMEWORK-023 through FRAMEWORK-026 and TASK-032 through TASK-043. The blocking sequence is lease recovery, the two adjacent authoring corrections, public 0.3.1 proof, a frozen controlled-evaluation protocol, blinded execution, published evidence, and only then broad promotion. JSON Schema/editor interoperability and roadmap-to-vision cardinality are explicit P1 follow-up; namespaced custom types remain evidence-gated P3 work.
- Updated TASK-018 and TASK-019 so examples require public-package proof and distribution cannot outrun hardening, evaluation, or tested positioning. The typed validated graph remains the product lead; a rename requires evidence rather than preference.
- Closed the audit loop without manufacturing duplicate work: deterministic create/update, inverse closure, adoption setup/check, and CI were already delivered by TASK-013 through TASK-015 and TASK-030/TASK-031. Internal package tests are not a consumer runtime requirement. Cosmetic title punctuation did not warrant a task, and unrestricted partial repair was rejected; TASK-036 owns the narrower mechanically isolated classification.
